    ...
    if (e.target.value === "") {
      ...
      async function loadPeople(name) {
        let response = await fetch(
          "http://193.187.174.137:8080/adding/userSearch/" + name,
          {
            method: "POST",
            headers: {
        ...
        if (response.status === 200) {
          if (type !== "chat") {
            setInputList(
              result.filter((item) => item.id != user.id && item.id != 0)
            );
          } else {
            ...
            setInputList(
              result1.filter((item) => item.id != user.id && item.id != 0)
            );
          }
          setLoad2(false);
        } else {
  ...
  return (
    ...
    >
      <div className={cn(common.modal)} style={{ width: "550px" }}>
        ...
        {peopleError && (
          <p className="loginError" style={{ top: "33px" }}>
            {peopleError}
          </p>
        )}
        <Formik
          ...
          onSubmit={(data) => {
            ...
            } else if (people.length===0 && type === "chat"){
              setPeopleError("Добавьте собеседника");
            } else {
              createChat(data);
            }
              ...
              async function create(data) {
                ...
                if (t === "PERSONAL") {
                  ...
                  let response1 = await fetch(
                    "http://193.187.174.137:8080/chat/getUser",
                    {
                      method: "GET",
                      headers: {
                        "Content-Type": "application/json",
                ...
                let response = await fetch(
                  "http://193.187.174.137:8080/chat/create",
                  {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      >
        <img
          src={item.avatar}
          style={{ height: "27px", width: "27px", marginRight: "15px", borderRadius:"100px" }}
        />
        <p>{item.username}</p>
      </div>
      <LineDivision />
...
const Person = ({ item, handleClose }) => {
  ...
  return (
    ...
    >
      <img
        src={item.avatar}
        style={{ height: "27px", width: "27px", marginRight: "6px", borderRadius:"100px" }}
      />
